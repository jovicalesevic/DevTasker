...
export function createUiRenderer(
  ...
  return {
    ...
    renderDiagnostics(): void {
      const state = getState();
      if (elements.diagSchemaVersion) elements.diagSchemaVersion.textContent = `${callbacks.t("diag.schema")}: v${options.schemaVersion}`;
      if (elements.diagSnapshots) elements.diagSnapshots.textContent = `${callbacks.t("diag.snapshots")}: ${callbacks.getSnapshotsCount()}/3`;
      if (elements.diagQueue) elements.diagQueue.textContent = `${callbacks.t("diag.queue")}: ${state.pendingSessions.length}`;
      if (elements.diagNetwork) elements.diagNetwork.textContent = `${callbacks.t("diag.network")}: ${navigator.onLine ? callbacks.t("sync.online") : callbacks.t("sync.offline")}`;
    }
  };
}
