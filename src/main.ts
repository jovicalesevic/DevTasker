type Priority = "hitno" | "bitno" | "moze";
type TaskStatus = "todo" | "in_progress" | "done";
type ReminderType = "due" | "stale";
type TabKey = "active" | "worklog" | "archive" | "weekly";
...
const sessionTaskSelect = byId<HTMLSelectElement>("sessionTaskSelect");
...
let activeDiffModalResolver: ((approved: boolean) => void) | null = null;
let activeDiffModalPreview: { compact: string; detailed: string; mode: "compact" | "detailed" } | null = null;
let lastDiffModalViewMode: "compact" | "detailed" = loadDiffViewMode();
...
function renderActiveTasks(): void {
  ...
  activeTaskList.querySelectorAll<HTMLButtonElement>('button[data-action="markDone"]').forEach((btn) => {
    btn.addEventListener("click", () => {
      changeTaskStatus(btn.dataset.id || "", "done");
    });
  });
  activeTaskList.querySelectorAll<HTMLButtonElement>('button[data-action="archive"]').forEach((btn) => {
    btn.addEventListener("click", () => {
  ...
  days.forEach((day) => {
    ...
    const list = grouped[day]
      .map((session) => {
        ...
        const durationMin = Math.max(1, Math.round((new Date(session.endedAt).getTime() - new Date(session.startedAt).getTime()) / 60_000));
        return `
...
function renderReminders(): void {
  ...
  reminderList.querySelectorAll<HTMLButtonElement>('button[data-action="dismissReminder"]').forEach((btn) => {
    btn.addEventListener("click", () => {
...
function validateTaskDraft(input: {
  ...
  priority: string;
  dueAt: string | null;
}): { title: string; description: string; priority: Priority; dueAt: string | null } | null {
...
function validatePendingSessionDraft(input: PendingSession): PendingSession | null {
  const minutes = Math.max(1, Math.round(input.minutes || 0));
  if (!input.id || !input.taskId || minutes < 1) return null;
  ...
  if (!isValidIsoDate(input.queuedAt)) return null;
...
function normalizeTask(value: unknown): Task | null {
  ...
  if (!t.id || !t.title || !t.projectId || !priority || !status) return null;
...
function normalizePendingSession(value: unknown): PendingSession | null {
  if (!value || typeof value !== "object") return null;
  const p = value as Partial<PendingSession>;
...
function normalizeReminder(value: unknown): Reminder | null {
  ...
  if (!r.id || !r.taskId || !type || !triggerAt) return null;
...
function normalizeWeeklySummary(value: unknown): WeeklySummary | null {
  if (!value || typeof value !== "object") return null;
  const w = value as Partial<WeeklySummary>;
  const weekStart = normalizeIsoDate(w.weekStart);
  const generatedAt = normalizeIsoDate(w.generatedAt);
  if (!w.id || !weekStart || !generatedAt) return null;
  return {
    id: String(w.id),
    weekStart,
...
function parsePriority(value: unknown): Priority | null {
  if (value === "hitno" || value === "bitno" || value === "moze") return value;
  return null;
}
...
function toggleDiffModalView(): void {
...
function saveDiffViewMode(mode: "compact" | "detailed"): void {
...
function renderDiffPreviewHtml(preview: string): string {
  return preview
    .split("\n")
    .map((line) => {
...
async function handleImportBackup(): Promise<void> {
  ...
  try {
    ...
    const confirmed = await showDiffModal(
      "Confirm import",
      `Mode: ${importMode}. ${modeNote}`,
      preview
    );
    if (!confirmed) {
...
type DiffMode = "replace" | "merge";

function buildDiffPreview(current: AppState, incoming: AppState, mode: DiffMode, detailLevel: "compact" | "detailed"): string {
...
function normalizeAppState(input: Partial<AppState>): AppState {
  ...
  const pendingSessions = (Array.isArray(input.pendingSessions) ? input.pendingSessions : [])
    ...
    .filter((item): item is PendingSession => item !== null);
  ...
  const weeklySummaries = (Array.isArray(input.weeklySummaries) ? input.weeklySummaries : [])
    ...
    .filter((item): item is WeeklySummary => item !== null);
...
function isPersistedEnvelope(value: unknown): value is PersistedStateEnvelope {
  ...
  const v = value as { schemaVersion?: unknown; data?: unknown };
  return typeof v.schemaVersion === "number" && typeof v.data === "object";
}

function migrateState(input: Partial<AppState>, _fromVersion: number): AppState {
...
function snapshotDataCounts(snapshot: SnapshotEntry): { projects: number; tasks: number; sessions: number } {
