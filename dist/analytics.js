...
export function averageLeadTimeHours(state) {
...
export function calculateWorklogStreak(sessions) {
    if (!sessions.length)
        return 0;
...
export function weeklyTrendText(state) {
    ...
    if (currentWeekDone > prevWeekDone)
        return `+${currentWeekDone - prevWeekDone} vs previous week`;
    if (currentWeekDone < prevWeekDone)
        return `-${prevWeekDone - currentWeekDone} vs previous week`;
    return "same as previous week";
}
export function topBlockers(sessions) {
...
export function latestTaskActivity(state, taskId) {
...
export function nextUpScore(task, lastActivity) {
    let score = 0;
...
export function predictTodayItems(state, scoreFn) {
